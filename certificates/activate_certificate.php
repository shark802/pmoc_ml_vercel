...
if (!isset($_POST['certificate_id']) || !isset($_POST['action']) || $_POST['action'] !== 'activate') {
...
try {
    ...
    if ($certificate['status'] !== 'revoked') {
        throw new Exception('Certificate is not revoked. Only revoked certificates can be activated.');
    }

    // Update certificate status to issued (reactivate)
    ...
    if (!$updateStmt->execute()) {
        throw new Exception('Failed to activate certificate');
    }

    // Create notification about activation
    $notificationContent = "Certificate reactivated for couple: " . $certificate['male_name'] . " & " . $certificate['female_name'];
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Certificate activated successfully for ' . $certificate['male_name'] . ' & ' . $certificate['female_name'],
    ...
    error_log("Certificate Activation Error: " . $e->getMessage());
    echo json_encode([
        'success' => false,
        'message' => 'Failed to activate certificate: ' . $e->getMessage()
    ]);
}
